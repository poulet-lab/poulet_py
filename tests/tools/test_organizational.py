...
from poulet_py.tools import check_or_create, define_folder_name, sanitize_path
...
class TestDefineFolderName:
    ...
    def test_with_date(self):
        ...
        assert (
            result == f"{datetime.now(timezone.utc).strftime('%Y%m%d')}_project"
        )
...
class TestSanitizePath:
    ...
    def test_add_timestamp(self):
        ...
        assert (
            str(result)
            == f"path/to/{datetime.now(timezone.utc).strftime('%Y%m%d')}_file.txt"
        )
