...
from helpers import detect_stimulus_frames, get_condition_from_attributes
...
def example_calculate_dff():
    ...
    baseline_map = wf.calculate_baseline(
        stimulus_start_frame=onset_frame, baseline_ms=500.0, fps=fps
    )
...
def example_roi_analysis():
    ...
    if dff_data is not None:
        ...
        roi_diameter = 150.0
        wf_trace = wf.calculate_trace_within_roi(data=dff_data, diameter=roi_diameter)
...
def example_create_movie():
    ...
    movie_path = wf.create_movie(data=None, output_path=None, fps=fps, cmap="gray")
...
def example_create_dff_movie():
    ...
    if dff_data is not None:
        dff_movie_path = wf.create_movie(
            data=dff_data, output_path=None, fps=fps, cmap="RdBu_r", vmin=-0.5, vmax=0.5
        )
...
def example_complete_workflow():
    ...
    if dff_data is not None:
        ...
        if wf_trace is not None:
            ...
            print(f"ROI center: ({roi[0]}, {roi[1]})")
            print(f"Trace statistics: mean={wf_trace.mean():.4f}, std={wf_trace.std():.4f}")
