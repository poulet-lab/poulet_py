...
def detect_stimulus_frames(
    ...
    smoothed = np.convolve(
        trace, np.ones(smooth_window) / smooth_window, mode='same'
    )
    ...
    smooth_deriv = np.convolve(
        derivative, np.ones(deriv_window) / deriv_window, mode='same'
    )
    
    search_start = int(sampling_rate * 3)
    search_end = int(sampling_rate * 10)
    if search_end > len(smooth_deriv):
        search_end = len(smooth_deriv)
    ...
    offset_indices_after_onset = offset_indices[
        offset_indices > (onset_sample + min_gap_samples)
    ]
    ...
    if pulse_duration_samples < min_pulse_samples:
        ...
        if pulse_duration_samples < min_pulse_samples:
            LOGGER.warning(
                f"No valid pulse found with duration >= {min_pulse_duration_ms:.1f}ms"
            )
            return None
    ...
    if plot:
        ...
        ax.axvline(x=result['onset_time'], color='red', linestyle='-', 
                  label='Onset', linewidth=2)
        ax.axvline(x=result['offset_time'], color='blue', linestyle='-', 
                  label='Offset', linewidth=2)
        ...
        ax = axes[1]
        ax.plot(time_axis, smooth_deriv, linewidth=1.0, alpha=0.8, 
               label='Derivative (smoothed)')
        ax.axhline(y=deriv_threshold, color='orange', linestyle='--', 
                  label=f'Rising threshold: +{deriv_threshold:.6f}', alpha=0.7)
        ax.axhline(y=-deriv_threshold, color='purple', linestyle='--', 
                  label=f'Falling threshold: -{deriv_threshold:.6f}', alpha=0.7)
